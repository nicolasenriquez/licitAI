...
type MercadoPublicoApiV1OcByDatePayload = {
  date: string;
};

@Injectable()
export class MercadoPublicoApiV1OcByDateService {
  private readonly logger = new Logger(MercadoPublicoApiV1OcByDateService.name);
  ...
  async run(payload: Record<string, unknown>): Promise<void> {
    ...
    try {
      ...
      this.logger.log(
        `Ingested ${persistenceResult.recordsFetched} V1 ordenes de compra for ${apiResponse.requestParams.fecha}`,
      );
    } catch (error) {
  ...
  private parsePayload(
  ...
  ): MercadoPublicoApiV1OcByDatePayload {
    const requestedDate = payload.date;

    if (!isNonEmptyString(requestedDate)) {
      throw new BadRequestException(
        'Mercado Publico V1 OC by-date payload requires a non-empty "date" string',
      );
    }
    ...
    if (Number.isNaN(parsedDate.getTime())) {
      throw new BadRequestException(
        'Mercado Publico V1 OC by-date payload "date" must be a valid ISO date',
      );
    }
