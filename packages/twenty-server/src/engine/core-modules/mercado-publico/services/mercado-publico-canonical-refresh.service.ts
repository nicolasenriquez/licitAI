...
type MercadoPublicoApiV2CompraAgilStagingRow = {
  id: string;
  source: string;
...
export class MercadoPublicoCanonicalRefreshService {
  ...
  async refreshV2CompraAgilFromApiSnapshot(
    rawApiPayloadId: string,
  ): Promise<number> {
    return this.coreDataSource.transaction(async (entityManager) => {
  ...
  private async selectLatestV2CompraAgilStagingRows(
    entityManager: EntityManager,
    rawApiPayloadId: string,
  ): Promise<MercadoPublicoApiV2CompraAgilStagingRow[]> {
    return entityManager.query<MercadoPublicoApiV2CompraAgilStagingRow[]>(
      `
  ...
  private async upsertCanonicalCompraAgil(
    entityManager: EntityManager,
    stagingRow: MercadoPublicoApiV2CompraAgilStagingRow,
  ): Promise<void> {
    await entityManager.query(
      `
