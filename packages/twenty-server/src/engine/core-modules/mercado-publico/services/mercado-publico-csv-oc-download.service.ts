...
import { MERCADO_PUBLICO_CSV_OC_DATASET } from 'src/engine/core-modules/mercado-publico/mercado-publico.constants';

type MercadoPublicoCsvOcDownloadPayload = {
  source_period: string;
  source_modality?: string;
};

@Injectable()
export class MercadoPublicoCsvOcDownloadService {
  private readonly logger = new Logger(MercadoPublicoCsvOcDownloadService.name);
    ...
    try {
      ...
      if (!settings.csvDownloadEnabled) {
        await this.mercadoPublicoPersistenceService.finalizeJobRun({
          ...
          finishedAt: new Date(),
          errorSummary:
            'CSV OC download disabled by MERCADO_PUBLICO_CSV_DOWNLOAD_ENABLED',
          recordsFetched: 0,
          recordsStaged: 0,
        ...
        this.logger.log(
          `CSV OC download skipped (csvDownloadEnabled=false) for period ${parsedPayload.source_period}`,
        );

        return;
      }

      if (!isNonEmptyString(settings.csvOrdenesDeCompraSourceUrl)) {
        throw new BadRequestException(
          'MERCADO_PUBLICO_CSV_OC_SOURCE_URL is not configured',
        );
      }
      ...
      this.logger.log(
        `Downloaded OC CSV ${parsedPayload.source_period}: ${result.fileSizeBytes}B deduped=${result.deduped}`,
      );
    } catch (error) {
  ...
  ): MercadoPublicoCsvOcDownloadPayload {
    if (!isNonEmptyString(payload.source_period)) {
      throw new BadRequestException(
        'Mercado Publico CSV OC download payload requires a non-empty "source_period" string (e.g. "2026-06")',
      );
    }
