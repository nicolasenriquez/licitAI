...
### PORTAROS en este baseline (desde rama congelada, archivo por archivo, sanitizados)
...
| `drivers/api/__tests__/fixtures/v2-compra-agil-*.json` (9 archivos) | idem | portados, sin tickets/secretos (`git grep -iE "ticket|[0-9]{9,}"` = 0) |
...
- Composición Docker Compose de `packages/twenty-docker` con almacenamiento aislado (volumen desechable); identidades `analista`/`operador` y credenciales SOLO por variables de entorno locales; nunca versionadas.
- Provisioner: reutiliza `data-seed-dev-workspace` para el workspace; identidades por env.
- Smoke autenticado: Playwright `packages/twenty-e2e-testing/tests/mercado-publico/baseline.spec.ts` — login real, bandera on → ruta V2 renderiza; bandera off → ruta ausente; evidencia (screenshot + trace + log de red) en directorio ignorado por Git; cero llamadas del navegador al proveedor.
