...
- G4 gate: closed on 2026-08-16 per operator decision. Tasks 2.3, 3.1, 3.2,
  3.3, 4.1, 4.2, and 4.3 are complete. The three-phase isolated harness,
  route-matrix proof, and focused gate suites passed, and durable `mp` state
  remained unchanged across enabled, disabled, and re-enabled deployments.
- G5 authorization: proceed with `mercado-publico-v2-legacy-retirement`.
  Retirement scope, zero-consumer proof, and certification remain governed by
  the G5 proposal; this change only lifts the G4 gate that blocked it.
- Rollback: the G4 route and rollback contract remains unchanged. When G5
  starts, rollback deploys the G4 gate-close revision `49d115e768` on
  `feat/mercado-publico-v2-baseline`.
...
- Interface: `REACT_APP_MERCADO_PUBLICO_V2_ENABLED` selects exactly one canonical
  ...
  of the G4-approved rollback reference recorded in `## Gate Status`.
...
- Human decision (2026-08-16): the G4 gate is closed. The operator reviewed
  the completed tasks and their evidence and authorizes G5
  `mercado-publico-v2-legacy-retirement` to proceed with deletion planning.
- Human decision: the live legacy rollback ends when G5 starts. The operator
  decision recorded in `## Gate Status` replaces the planned standalone
  release record and rollback tag; G5 rollback deploys the gate-close
  revision named there.
