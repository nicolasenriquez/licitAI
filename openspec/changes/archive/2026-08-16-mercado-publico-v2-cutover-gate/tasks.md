...
- [x] 2.4 Document exact local cutover, rollback, re-enable, evidence-retention,
  ...
  that G5 rollback deploys the G4-approved rollback reference recorded at gate
  close.
...
- [x] 4.2 Close the G4 gate with an explicit operator decision that approves or
  rejects G5 based on complete local evidence. Record the approved rollback
  reference for G5. Preserve alias and all retained consumers until G5 begins,
  regardless of decision.
  Traceability: Group G4; Slice S3; Issue 31; Acceptance AC 31.1 through AC 31.6.
  Notes: 2026-08-16. Closed via operator decision recorded in `proposal.md`
  `## Gate Status`; rollback reference is gate-close commit `49d115e768`. No
  standalone release record or tag was created; the decision supersedes both.
