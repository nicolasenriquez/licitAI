...
> **licitAI precedence:** this is a Vite SPA — React Server Components and Next.js server actions do not apply.
> State management is Jotai + Apollo Client; styling is Linaria only; i18n is Lingui; functional components with named exports only ([docs/standards/react-standard.md](../../docs/standards/react-standard.md)).
> External-store and data-fetching decision trees defer to the repository's established conventions (.cursor/rules/react-state-management.mdc).
