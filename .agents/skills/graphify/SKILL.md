...
**MANDATORY: You MUST use the active harness dispatch adapter here. Reading
files yourself one-by-one is forbidden - it is 5-10x slower. If you do not use
the adapter you are doing this wrong.**
...
**Step B2 - Dispatch ALL subagents in a single message using the active harness**

Read exactly one co-located harness dispatch reference and follow it without
mixing protocols. The selected reference defines parallel dispatch, result
collection, and the extraction prompt to load:

- `references/harness-dispatch-codex.md`
- `references/harness-dispatch-opencode.md`

Select the reference from the active harness, not from the source files being
processed. Parse each result as JSON, accumulate nodes/edges/hyperedges, and
write to `graphify-out/.graphify_semantic_new.json`. Apply the disk-based
checks in Step B3 only when the selected adapter writes chunk files.

Subagent prompt template:

Load the extraction prompt named by the selected harness dispatch reference.
It contains the rules, node-ID format, confidence rubric, hyperedge and vision
rules, and JSON schema for that harness. Load it only when at least one chunk
holds a doc, paper, or image; a pure-code corpus skips Part B and never reads
it. Pass the selected prompt verbatim with FILE_LIST, CHUNK_NUM, TOTAL_CHUNKS,
and DEEP_MODE substituted.
...
dispatched chunk completes, read the real token counts from the selected
harness result's `usage` field and write them back into the chunk JSON before
