...
> **licitAI precedence:** use this skill for safety principles only (expand-contract, reversibility, zero-downtime).
> All schema changes go through the TypeORM instance-command machinery: `npx nx run twenty-server:database:migrate:generate --name <name> --type <fast|slow>` per [packages/twenty-server/docs/UPGRADE_COMMANDS.md](../../../packages/twenty-server/docs/UPGRADE_COMMANDS.md).
> Prisma/Drizzle/raw-DDL examples do not apply. Never rewrite committed up/down logic — append only.
